...
```{r, fig.margin=TRUE, fig.cap = 'Contour of prices in Liverpool', eval=FALSE}
...
```{r, fig.margin=TRUE, fig.cap = 'Skewness of prices in Liverpool', eval=FALSE}
qplot(data=xyz, x=z, geom='density')
```

Let us then take the logarithm before we plot the surface:

```{r, fig.fullwidth=TRUE, fig.cap = 'Surface of log-prices in Liverpool', eval=FALSE}
xyz['lz'] <- log(xyz$z)
base <- ggplot(data=xyz, aes(x=x, y=y))
surface <- base +
