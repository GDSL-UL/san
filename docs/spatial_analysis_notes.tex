...
\begin{itemize}
\item
  \citet{Snijders_Bosker_2012_book}
\item
  \citet{bristol}
\end{itemize}
...
\hypertarget{multilevel-data-structures}{%
\chapter{Multilevel Data Structures}\label{multilevel-data-structures}}
...
\section{Long vs wide format}\label{long-vs-wide-format}}

\hypertarget{single-level-regression-models}{%
\chapter{Single-level Regression Models}\label{single-level-regression-models}}

\[y_{i} = \beta_{0} + \beta_{1} x_{1i} + e_{i}\]

\textbf{Example: estimate regression model}
...
\item
  Only captures on macro \emph{vs} micro processes

  \begin{itemize}
  \tightlist
  \item
    A macro approach: enables understanding of higher level unit analysis, not individuals within these units \textgreater{} Ecological fallacy
  \item
    A micro approach: enables understanding of individuals without acknoledgement of dependencies between individuals within the same group or spatial unit \textgreater{} Underestimation of standard errors
  \end{itemize}
\item
  May lead to drawn erroneous conclusions frome the identification of misleading relationships
\end{itemize}

\hypertarget{adding-group-fixed-effects}{%
\section{Adding Group Fixed Effects}\label{adding-group-fixed-effects}}
...
In R, there are several Packages implementing multilevel models. This tutorial will show how to use the \texttt{lme4} Package (short for linear mixed effect models) in R to fit MLM and use the companion \texttt{merTools} Package to neatly analyse the model output from \texttt{lme4}. This tutorial will cover getting set up and running a few random interpet multilevel models and interpret model estiamtion results.
...
\textbf{What is the hierarchy of the data?}
